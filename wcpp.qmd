...
data.frame(
  ...
  kbl(col.names = NA, align = "lrr", escape = F, table.attr = "data-quarto-disable-processing=true",
      caption = paste(var_ikon$k_penge, "Præmiepenge til WCPP")) %>%
...
data.frame(
  ...
  "Spiller" = c(
    ...
    "Maxim Shmyrev"),
  check.names = F) %>%
  kbl(col.names = NA, align = "cll", escape = F, table.attr = "data-quarto-disable-processing=true",
      caption = paste(var_ikon$k_pokal, "WCPP-vindere statistik")) %>%
  kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
  row_spec(0, background = "#1C2833", color = "#FFFFFF") %>%
  gsub("\\{\\{&lt;|&lt;", "{{<", .) %>% gsub("&gt;}}", ">}}", .)
```
